...
#constructing different parameters
...
#function which checks that each vehicle has a unique matricul and updating the matricul if not.
  ...
  for (i in 1:(nrow(vehicle) - 1)) {
    for (j in (i + 1):nrow(vehicle)) {
      if (vehicle$vehicle_id[i] == vehicle$vehicle_id[j]) {
        ...
        vehicle$vehicle_id[i] <- readline(prompt = "first vehicle.... \n")
        vehicle$vehicle_id[j] <- readline(prompt = "second vehicle.... \n")
...
#function which checks that a vehicle is place on an empty position on the road and updating it position if not.
...
#Function that set the parameters of each vehicle for simulation
...
#Show each vehicle at its initial position and return the updated status of its location
Display_at_initial <- function(vehicle, point_east, point_west, point_north, point_south){
  for (i in 1:nrow(vehicle)){
    ...
    if(vehicle$Vehicle_Origine[i] == "east"){
      print(point_east)
      points(point_east$X_coordinate[j], point_east$Y_coordinate[j],pch=20, col="red", cex=2)
      point_east$Occupied[j] <- TRUE

    }
    if(vehicle$Vehicle_Origine[i] == "west"){
      points(point_west$X_coordinate[j], point_west$Y_coordinate[j],pch=20, col="blue", cex=2)
      point_west$Occupied[j] <- TRUE
    }
    if(vehicle$Vehicle_Origine[i] == "north"){
      points(point_north$X_coordinate[j], point_north$Y_coordinate[j],pch=20, col="magenta", cex=2)
      point_north$Occupied[j] <- TRUE
    }
    if(vehicle$Vehicle_Origine[i] == "south"){
      points(point_south$X_coordinate[j], point_south$Y_coordinate[j],pch=20, col="cyan", cex=2)
...
# procedure that show up the simulation
      ...
      if (vehicle$Vehicle_Origine[i] == "east") {
        ...
        if (vehicle$vehicle_initial[i]+1 < 20) {
          ...
          if (!point_E1$Occupied[z+1]) {
              points(point_E1$X_coordinate[z + 1],point_E1$Y_coordinate[z + 1],pch = 20,col = "red",cex = 2)
              point_E1$Occupied[z] <- FALSE
              point_E1$Occupied[z+1] <- TRUE
              vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i]+1
          } else if (point_E1$Occupied[z + 1]) {
            points(point_E1$X_coordinate[z],point_E1$Y_coordinate[z],pch = 20,col = "red",cex = 2)
          ...
          if(vehicle$Vehicle_Destination[i]== "return"){
           if(vehicle$vehicle_initial[i]==19){
             ...
             if(!point_R$Occupied[1]){
               points(point_R$X_coordinate[1],point_R$Y_coordinate[1],pch = 20,col = "red",cex = 2)
             ...
             z <- vehicle$vehicle_initial[i]
             if(!point_R$Occupied[5]){ #check middle point 5 if free
               points(point_R$X_coordinate[5],point_R$Y_coordinate[5],pch = 20,col = "red",cex = 2)
               ...
               points(point_R$X_coordinate[2],point_R$Y_coordinate[2],pch = 20,col = "red",cex = 2)
               point_R$Occupied[2] <- TRUE
               point_R$Occupied[1] <- FALSE
               path1$value[1] <- TRUE
               vehicle$vehicle_initial[i] <- 21
             }else if(!point_R$Occupied[4]){
               points(point_R$X_coordinate[4],point_R$Y_coordinate[4],pch = 20,col = "red",cex = 2)
             ...
             if(!point_E2$Occupied[1]){
               points(point_E2$X_coordinate[1],point_E2$Y_coordinate[1],pch = 20,col = "red",cex = 2)
             ...
             if(!point_E2$Occupied[vehicle$vehicle_initial[i]-21]){
               points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-21],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "red",cex = 2)
               point_E2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
               point_E2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
               vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
             }else{
               points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "red",cex = 2)
                ...
                if(!point_R$Occupied[2]){
                  ...
                  if(path2$value[1] == FALSE){
                   if(!point_W2$Occupied[1]){
                    points(point_W2$X_coordinate[1],point_W2$Y_coordinate[1],pch = 20,col = "red",cex = 2)
                    point_R$Occupied[2] <- FALSE
                  ...
                  if(!point_W2$Occupied[vehicle$vehicle_initial[i]-20]){
                    points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-20],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-20],pch = 20,col = "red",cex = 2)
                    point_W2$Occupied[vehicle$vehicle_initial[i]-20-1] <- FALSE
                    point_E2$Occupied[vehicle$vehicle_initial[i]-20] <- TRUE
                    vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                  }else{
                    points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-20-1],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-20-1],pch = 20,col = "red",cex = 2)
                ...
                if(vehicle$vehicle_initial[i]==19){
                  ...
                  z <- vehicle$vehicle_initial[i]
                    if(path1$value_left[1]){
                      if(!point_R$Occupied[3]){
                        points(point_R$X_coordinate[3],point_R$Y_coordinate[3],pch = 20,col = "red",cex = 2)
                  ...
                  if(vehicle$vehicle_initial[i]==22){
                    ...
                    if(!point_N2$Occupied[1]){
                      points(point_N2$X_coordinate[1],point_N2$Y_coordinate[1],pch = 20,col = "red",cex = 2)
                    ...
                    if(!point_N2$Occupied[vehicle$vehicle_initial[i]-21]){
                      points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-21],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "red",cex = 2)
                      point_N2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
                      point_N2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
                      vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                    }else{
                      points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "red",cex = 2)
                  ...
                  if(vehicle$vehicle_initial[i]==19){
                    ...
                    if(vehicle$vehicle_initial[i]==20){
                      ...
                      if(!point_S2$Occupied[1]){
                        points(point_S2$X_coordinate[1],point_S2$Y_coordinate[1],pch = 20,col = "red",cex = 2)
                        point_R$Occupied[1] <- FALSE
                      ...
                      if(!point_S2$Occupied[vehicle$vehicle_initial[i]-19]){
                        points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-19],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-19],pch = 20,col = "red",cex = 2)
                        point_S2$Occupied[vehicle$vehicle_initial[i]-19-1] <- FALSE
                        point_S2$Occupied[vehicle$vehicle_initial[i]-19] <- TRUE
                        vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                      }else{
                        points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-19-1],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-19-1],pch = 20,col = "red",cex = 2)
        ...
        if(vehicle$Vehicle_Origine[i] == "west"){
          ...
          if (vehicle$vehicle_initial[i]+1 < 20) {
            ...
            if (!point_W1$Occupied[z+1]) {
              points(point_W1$X_coordinate[z + 1],point_W1$Y_coordinate[z + 1],pch = 20,col = "blue",cex = 2)
              point_W1$Occupied[z] <- FALSE
              point_W1$Occupied[z+1] <- TRUE
              vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i]+1
            } else if (point_W1$Occupied[z + 1]) {
              points(point_W1$X_coordinate[z],point_W1$Y_coordinate[z],pch = 20,col = "blue",cex = 2)
            ...
            if(vehicle$Vehicle_Destination[i]== "return"){
              if(vehicle$vehicle_initial[i]==19){
                z <- vehicle$vehicle_initial[i]
                if(!point_R$Occupied[3]){
                  points(point_R$X_coordinate[3],point_R$Y_coordinate[3],pch = 20,col = "blue",cex = 2)
                  point_R$Occupied[3] <- TRUE
                  point_W1$Occupied[z] <- FALSE
                  vehicle$vehicle_initial[i] <- 20
                }else {
                ...
                z <- vehicle$vehicle_initial[i]
                if(!point_R$Occupied[5]){
                  points(point_R$X_coordinate[5],point_R$Y_coordinate[5],pch = 20,col = "blue",cex = 2)
                  point_R$Occupied[5] <- TRUE
                  point_R$Occupied[3] <- FALSE
                  vehicle$vehicle_initial[i] <- 21
                }else if(!point_R$Occupied[4]){
                  points(point_R$X_coordinate[4],point_R$Y_coordinate[4],pch = 20,col = "blue",cex = 2)
                  point_R$Occupied[4] <- TRUE
                  point_R$Occupied[3] <- FALSE
                  path1$value[2] <- TRUE
                }else if(!point_R$Occupied[2]){
                  points(point_R$X_coordinate[2],point_R$Y_coordinate[2],pch = 20,col = "blue",cex = 2)
                  point_R$Occupied[2] <- TRUE
                  point_R$Occupied[3] <- FALSE
                  vehicle$vehicle_initial[i] <- 22
                }else{
                ...
                if(vehicle$vehicle_initial[i]==22){
                  ...
                  if(!point_W2$Occupied[1]){
                    points(point_W2$X_coordinate[1],point_W2$Y_coordinate[1],pch = 20,col = "blue",cex = 2)
                    point_R$Occupied[2] <- FALSE
                    point_W2$Occupied[1] <- TRUE
                    vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                  }else{
                  ...
                  if(!point_W2$Occupied[vehicle$vehicle_initial[i]-21]){
                    points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-21],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "blue",cex = 2)
                    point_W2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
                    point_W2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
                    vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                  }else{
                    points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "blue",cex = 2)
              ...
              if(vehicle$Vehicle_Destination[i] == "straight"){
                ...
                if(vehicle$vehicle_initial[i]==19){
                  ...
                  if(!point_E2$Occupied[1]){
                    points(point_E2$X_coordinate[1],point_E2$Y_coordinate[1],pch = 20,col = "blue",cex = 2)
                    point_R$Occupied[4] <- FALSE
                    point_E2$Occupied[1] <- TRUE
                    vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                  }else{
                  ...
                  if(!point_E2$Occupied[vehicle$vehicle_initial[i]-20]){
                    points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-20],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-20],pch = 20,col = "blue",cex = 2)
                    point_E2$Occupied[vehicle$vehicle_initial[i]-20-1] <- FALSE
                    point_E2$Occupied[vehicle$vehicle_initial[i]-20] <- TRUE
                    vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                  }else{
                    points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-20-1],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-20-1],pch = 20,col = "blue",cex = 2)
                    point_W2$Occupied[vehicle$vehicle_initial[i]-20-1] <- TRUE
                  }
                }
                ...
                if(vehicle$Vehicle_Destination[i] == "left"){
                  ...
                  if(vehicle$vehicle_initial[i]==19){
                    ...
                    if(path1$value_left[2]){
                      if(!point_R$Occupied[1]){
                        points(point_R$X_coordinate[1],point_R$Y_coordinate[1],pch = 20,col = "blue",cex = 2)
                        point_R$Occupied[1] <- TRUE
                        point_R$Occupied[5] <- FALSE
                        vehicle$vehicle_initial[i] <- 22
                      }
                    ...
                    if(vehicle$vehicle_initial[i]==22){
                      ...
                      if(!point_S2$Occupied[1]){
                        points(point_S2$X_coordinate[1],point_S2$Y_coordinate[1],pch = 20,col = "blue",cex = 2)
                        point_R$Occupied[1] <- FALSE
                        point_S2$Occupied[1] <- TRUE
                        vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                      }else{
                      ...
                      if(!point_S2$Occupied[vehicle$vehicle_initial[i]-21]){
                        points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-21],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "blue",cex = 2)
                        point_S2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
                        point_S2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
                        vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                      }else{
                        points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "blue",cex = 2)
                  ...
                  if(vehicle$Vehicle_Destination[i] == "right"){
                    ...
                    if(vehicle$vehicle_initial[i]==19){
                      ...
                      if(vehicle$vehicle_initial[i]==20){
                        ...
                        if(!point_N2$Occupied[1]){
                          points(point_N2$X_coordinate[1],point_N2$Y_coordinate[1],pch = 20,col = "blue",cex = 2)
                          point_R$Occupied[3] <- FALSE
                          point_N2$Occupied[1] <- TRUE
                          vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                        }else{
                        ...
                        if(!point_N2$Occupied[vehicle$vehicle_initial[i]-19]){
                          points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-19],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-19],pch = 20,col = "blue",cex = 2)
                          point_N2$Occupied[vehicle$vehicle_initial[i]-19-1] <- FALSE
                          point_N2$Occupied[vehicle$vehicle_initial[i]-19] <- TRUE
                          vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                        }else{
                          points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-19-1],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-19-1],pch = 20,col = "blue",cex = 2)
          ...
          if(vehicle$Vehicle_Origine[i] == "north"){
            if (vehicle$vehicle_initial[i]+1 < 20) {
              ...
              if (!point_N1$Occupied[z+1]) {
                points(point_N1$X_coordinate[z + 1],point_N1$Y_coordinate[z + 1],pch = 20,col = "magenta",cex = 2)
                point_N1$Occupied[z] <- FALSE
                point_N1$Occupied[z+1] <- TRUE
                vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i]+1
              } else if (point_N1$Occupied[z + 1]) {
                points(point_N1$X_coordinate[z],point_N1$Y_coordinate[z],pch = 20,col = "magenta",cex = 2)
              ...
              if(vehicle$Vehicle_Destination[i]== "return"){
                if(vehicle$vehicle_initial[i]==19){
                  z <- vehicle$vehicle_initial[i]
                  if(!point_R$Occupied[4]){
                    points(point_R$X_coordinate[4],point_R$Y_coordinate[4],pch = 20,col = "magenta",cex = 2)
                    point_R$Occupied[4] <- TRUE
                    point_N1$Occupied[z] <- FALSE
                    vehicle$vehicle_initial[i] <- 20
                  }else {
                  ...
                  z <- vehicle$vehicle_initial[i]
                  if(!point_R$Occupied[5]){
                    points(point_R$X_coordinate[5],point_R$Y_coordinate[5],pch = 20,col = "magenta",cex = 2)
                    point_R$Occupied[5] <- TRUE
                    point_R$Occupied[4] <- FALSE
                    vehicle$vehicle_initial[i] <- 21
                  }else if(!point_R$Occupied[1]){
                    points(point_R$X_coordinate[1],point_R$Y_coordinate[1],pch = 20,col = "magenta",cex = 2)
                    point_R$Occupied[1] <- TRUE
                    point_R$Occupied[4] <- FALSE
                    path1$value[3] <- TRUE
                  }else if(!point_R$Occupied[3]){
                    points(point_R$X_coordinate[3],point_R$Y_coordinate[3],pch = 20,col = "magenta",cex = 2)
                    point_R$Occupied[3] <- TRUE
                    point_R$Occupied[4] <- FALSE
                    vehicle$vehicle_initial[i] <- 22
                  }else{
                  ...
                  if(vehicle$vehicle_initial[i]==22){
                    ...
                    if(!point_N2$Occupied[1]){
                      points(point_N2$X_coordinate[1],point_N2$Y_coordinate[1],pch = 20,col = "magenta",cex = 2)
                      point_R$Occupied[3] <- FALSE
                      point_N2$Occupied[1] <- TRUE
                      vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                    }else{
                    ...
                    if(!point_N2$Occupied[vehicle$vehicle_initial[i]-21]){
                      points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-21],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "magenta",cex = 2)
                      point_N2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
                      point_N2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
                      vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                    }else{
                      points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "magenta",cex = 2)
                      point_N2$Occupied[vehicle$vehicle_initial[i]-21-1] <- TRUE
                    }
                  }
              }else
                if(vehicle$Vehicle_Destination[i] == "straight"){
                  ...
                  if(path2$value[3]==FALSE){
                    if(!point_S2$Occupied[1]){
                      points(point_S2$X_coordinate[1],point_S2$Y_coordinate[1],pch = 20,col = "magenta",cex = 2)
                      point_R$Occupied[1] <- FALSE
                      point_S2$Occupied[1] <- TRUE
                      vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                    }else{
                    ...
                    if(!point_S2$Occupied[vehicle$vehicle_initial[i]-20]){
                      points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-20],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-20],pch = 20,col = "magenta",cex = 2)
                      point_S2$Occupied[vehicle$vehicle_initial[i]-20-1] <- FALSE
                      point_S2$Occupied[vehicle$vehicle_initial[i]-20] <- TRUE
                      vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                    }else{
                      points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-20-1],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-20-1],pch = 20,col = "magenta",cex = 2)
                  ...
                  if(vehicle$Vehicle_Destination[i] == "left"){
                    ...
                    if(vehicle$vehicle_initial[i]==19){
                      ...
                      if(path1$value_left[3]){
                        if(!point_R$Occupied[2]){
                          points(point_R$X_coordinate[2],point_R$Y_coordinate[2],pch = 20,col = "magenta",cex = 2)
                          point_R$Occupied[2] <- TRUE
                          point_R$Occupied[5] <- FALSE
                          vehicle$vehicle_initial[i] <- 22
                        }
                        ...
                        #procedure that displace a vehicle on W2
                        if(!point_W2$Occupied[1]){
                          points(point_W2$X_coordinate[1],point_W2$Y_coordinate[1],pch = 20,col = "magenta",cex = 2)
                          point_W2$Occupied[1] <- TRUE
                          point_R$Occupied[2] <- FALSE
                          vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                        }else{
                        ...
                        if(!point_W2$Occupied[vehicle$vehicle_initial[i]-21]){
                          points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-21],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "magenta",cex = 2)
                          point_W2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
                          point_W2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
                          vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                        }else{
                          points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "magenta",cex = 2)
                          point_W2$Occupied[vehicle$vehicle_initial[i]-21-1] <- TRUE
                        }
                      }
                    ...
                    if(vehicle$Vehicle_Destination[i] == "right"){
                      ...
                      if(vehicle$vehicle_initial[i]==19){
                        ...
                        if(vehicle$vehicle_initial[i]==20){
                          ...
                          if(!point_E2$Occupied[1]){
                            points(point_E2$X_coordinate[1],point_E2$Y_coordinate[1],pch = 20,col = "magenta",cex = 2)
                            point_R$Occupied[4] <- FALSE
                            point_E2$Occupied[1] <- TRUE
                            vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                          }else{
                          ...
                          if(!point_E2$Occupied[vehicle$vehicle_initial[i]-19]){
                            points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-19],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-19],pch = 20,col = "magenta",cex = 2)
                            point_E2$Occupied[vehicle$vehicle_initial[i]-19-1] <- FALSE
                            point_E2$Occupied[vehicle$vehicle_initial[i]-19] <- TRUE
                            vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                          }else{
                            points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-19-1],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-19-1],pch = 20,col = "magenta",cex = 2)
            ...
            if(vehicle$Vehicle_Origine[i] == "south"){
              ...
              if (vehicle$vehicle_initial[i]+1 < 20) {
                ...
                if (!point_S1$Occupied[z+1]) {
                  points(point_S1$X_coordinate[z + 1],point_S1$Y_coordinate[z + 1],pch = 20,col = "cyan",cex = 2)
                  point_S1$Occupied[z] <- FALSE
                  point_S1$Occupied[z+1] <- TRUE
                  vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i]+1
                } else if (point_S1$Occupied[z + 1]) {
                  points(point_S1$X_coordinate[z],point_S1$Y_coordinate[z],pch = 20,col = "cyan",cex = 2)
                ...
                if(vehicle$Vehicle_Destination[i]== "return"){
                  if(vehicle$vehicle_initial[i]==19){
                    z <- vehicle$vehicle_initial[i]
                    if(!point_R$Occupied[2]){
                      points(point_R$X_coordinate[2],point_R$Y_coordinate[2],pch = 20,col = "cyan",cex = 2)
                      point_R$Occupied[2] <- TRUE
                      point_S1$Occupied[z] <- FALSE
                      vehicle$vehicle_initial[i] <- 20
                    }else {
                    ...
                    z <- vehicle$vehicle_initial[i]
                    if(!point_R$Occupied[5]){
                      points(point_R$X_coordinate[5],point_R$Y_coordinate[5],pch = 20,col = "cyan",cex = 2)
                      point_R$Occupied[5] <- TRUE
                      point_R$Occupied[2] <- FALSE
                      vehicle$vehicle_initial[i] <- 21
                    }else if(!point_R$Occupied[3]){
                      points(point_R$X_coordinate[3],point_R$Y_coordinate[3],pch = 20,col = "cyan",cex = 2)
                      point_R$Occupied[3] <- TRUE
                      point_R$Occupied[2] <- FALSE
                      path1$value[4]<-TRUE
                    }else if(!point_R$Occupied[1]){
                      points(point_R$X_coordinate[1],point_R$Y_coordinate[1],pch = 20,col = "cyan",cex = 2)
                      point_R$Occupied[1] <- TRUE
                      point_R$Occupied[2] <- FALSE
                      vehicle$vehicle_initial[i] <- 22
                    }else{
                    ...
                    if(vehicle$vehicle_initial[i]==22){
                      ...
                      if(!point_S2$Occupied[1]){
                        points(point_S2$X_coordinate[1],point_S2$Y_coordinate[1],pch = 20,col = "cyan",cex = 2)
                        point_R$Occupied[1] <- FALSE
                        point_S2$Occupied[1] <- TRUE
                        vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                      }else{
                      ...
                      if(!point_S2$Occupied[vehicle$vehicle_initial[i]-21]){
                        points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-21],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "cyan",cex = 2)
                        point_S2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
                        point_S2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
                        vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                      }else{
                        points(point_S2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_S2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "cyan",cex = 2)
                        point_S2$Occupied[vehicle$vehicle_initial[i]-21-1] <- TRUE
                      }
                    }
                }else
                  if(vehicle$Vehicle_Destination[i] == "straight"){
                    ...
                    if(vehicle$vehicle_initial[i]==19){
                      ...
                      if(!point_N2$Occupied[1]){
                        points(point_N2$X_coordinate[1],point_N2$Y_coordinate[1],pch = 20,col = "cyan",cex = 2)
                        point_R$Occupied[3] <- FALSE
                        point_N2$Occupied[1] <- TRUE
                        vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                      }else{
                      ...
                      if(!point_N2$Occupied[vehicle$vehicle_initial[i]-20]){
                        points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-20],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-20],pch = 20,col = "cyan",cex = 2)
                        point_N2$Occupied[vehicle$vehicle_initial[i]-20-1] <- FALSE
                        point_N2$Occupied[vehicle$vehicle_initial[i]-20] <- TRUE
                        vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                      }else{
                        points(point_N2$X_coordinate[vehicle$vehicle_initial[i]-20-1],point_N2$Y_coordinate[vehicle$vehicle_initial[i]-20-1],pch = 20,col = "cyan",cex = 2)
                    ...
                    if(vehicle$Vehicle_Destination[i] == "left"){
                      ...
                      if(vehicle$vehicle_initial[i]==19){
                        ...
                        if(path1$value_left[4]){
                          if(!point_R$Occupied[4]){
                            points(point_R$X_coordinate[4],point_R$Y_coordinate[4],pch = 20,col = "cyan",cex = 2)
                            point_R$Occupied[4] <- TRUE
                            point_R$Occupied[5] <- FALSE
                            vehicle$vehicle_initial[i] <- 22
                          }
                        ...
                        if(vehicle$vehicle_initial[i]==22){
                          ...
                          if(!point_E2$Occupied[1]){
                            points(point_E2$X_coordinate[1],point_E2$Y_coordinate[1],pch = 20,col = "cyan",cex = 2)
                            point_R$Occupied[4] <- FALSE
                            point_E2$Occupied[1] <- TRUE
                            vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                          }else{
                          ...
                          if(!point_E2$Occupied[vehicle$vehicle_initial[i]-21]){
                            points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-21],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-21],pch = 20,col = "cyan",cex = 2)
                            point_E2$Occupied[vehicle$vehicle_initial[i]-21-1] <- FALSE
                            point_E2$Occupied[vehicle$vehicle_initial[i]-21] <- TRUE
                            vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                          }else{
                            points(point_E2$X_coordinate[vehicle$vehicle_initial[i]-21-1],point_E2$Y_coordinate[vehicle$vehicle_initial[i]-21-1],pch = 20,col = "cyan",cex = 2)
                            point_E2$Occupied[vehicle$vehicle_initial[i]-21-1] <- TRUE
                          }
                        }
                      ...
                      if(vehicle$Vehicle_Destination[i] == "right"){
                        ...
                        if(vehicle$vehicle_initial[i]==19){
                          ...
                          if(vehicle$vehicle_initial[i]==20){
                            #procedure that displace a vehicle on W2
                            if(!point_W2$Occupied[1]){
                              points(point_W2$X_coordinate[1],point_W2$Y_coordinate[1],pch = 20,col = "cyan",cex = 2)
                              point_R$Occupied[2] <- FALSE
                              point_W2$Occupied[1] <- TRUE
                              vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                            }else{
                            ...
                            if(!point_W2$Occupied[vehicle$vehicle_initial[i]-19]){
                              points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-19],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-19],pch = 20,col = "cyan",cex = 2)
                              point_W2$Occupied[vehicle$vehicle_initial[i]-19-1] <- FALSE
                              point_W2$Occupied[vehicle$vehicle_initial[i]-19] <- TRUE
                              vehicle$vehicle_initial[i] <- vehicle$vehicle_initial[i] + 1
                            }else{
                              points(point_W2$X_coordinate[vehicle$vehicle_initial[i]-19-1],point_W2$Y_coordinate[vehicle$vehicle_initial[i]-19-1],pch = 20,col = "cyan",cex = 2)
...
#test of procedures and functions
roads_four_ways(x, y)

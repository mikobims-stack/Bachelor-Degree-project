...
destination <- c("left", "right", "return")
origine <- c("east", "north", "south")
road_axes <- c("east", "north", "south")
...
#' Constructing points on the EAST,  NORTH and SOUTH road and the ROUNDABOUT.
#'
#' This portion of the code creates a list of displacement point starting from east, north and south.
#' this procedure is automatically call once the simulation of a particular vehicle is lunched.
#'
#' @param  it uses the x and y coordinates from east to roundabout (x) and from north to south(y).
...
#' @export
Enter_parameter_Y <- function(vehicle) {
...
#' @return  A data frame containing the entered parameters by the user
#' @export
check_Initial_Y <- function(vehicle) {
  if(nrow(vehicle)>1){
  for (i in 1:(nrow(vehicle) - 1)) {
    for (j in (i + 1):nrow(vehicle)) {
...
#' @export
Display_at_initial_Y <- function(vehicle, point_east, point_north, point_south){
  ...
  return(list(point_east =  point_east,point_north =  point_north,point_south =  point_south))
}
...
Simulation_Y <- function(x,y,vehicle,point_E1, point_E2, point_N1, point_N2, point_S1, point_S2, point_R){
  repeat{
    ...
    for (i in 1:nrow(vehicle)) {
      ...
      if(vehicle$Vehicle_Origine[i] == "south"){
        ...
        if (vehicle$vehicle_initial[i]+1 < 20) {
          ...
          } else if(vehicle$Vehicle_Destination[i] == "right"){
          ...
          } else if(vehicle$Vehicle_Destination[i] == "left"){
      ...
      }  # This closes the if(vehicle$Vehicle_Origine[i] == "south") block
...
vehicle <- Enter_parameter_Y(vehicle)
#verifies the Matricul and initial position
vehicle <- check_Id_Y(vehicle)
vehicle <- check_Initial_Y(vehicle)

#it display the vehicles at it initial and save an
Update_state <- Display_at_initial_Y(vehicle,
                   Default_point_on_road_east_1,
